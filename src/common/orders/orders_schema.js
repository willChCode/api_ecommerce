...
const orderSchema = z.object({
  user: z.string().min(1, 'Required').superRefine(( val, ctx ) => {
  ...
  orderItems: z.object({
    ...
    slug: z.string().min(1).superRefine((val, ctx) => {
  ...
  }).array().min(1, 'Al menos un product es requerido en el carrito para crear una orden'),
    ...
    numberOfItems: z.number().min(1, 'Al menos un product es requerido en el carrito para crear una orden'),
