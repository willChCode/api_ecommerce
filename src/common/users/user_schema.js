...
const registerSchema = z.object({
  name: z.string().min(1, 'Required').max(30), 
  age: z.number().min(1, 'Required').min(10, 'Los menores de edad no se admiten').max(100, 'No queremos viejos'), 
  email: z.string().min(1, 'Required').email(),
  password: z.string().min(1, 'Required').max(30).trim().superRefine((val, ctx) => {
